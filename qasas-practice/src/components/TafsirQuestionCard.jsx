...
export default function TafsirQuestionCard({ question, showFeedback, currentAnswer, onAnswer }) {
  ...
  const surahLabel = question.surahName.startsWith('Surah ')
    ? question.surahName
    : `Surah ${question.surahName}`;

  return (
    <div className="tafsir-question-card">
      <div className="tafsir-source-row">
        <span className="tafsir-source-badge">{surahLabel}</span>
        <span className="tafsir-ayah-number">Ayah {question.ayah}</span>
      </div>
      <h2 className="tafsir-question-prompt">Choose the translation</h2>
      <div className="tafsir-arabic-text" dir="rtl" lang="ar">
        {question.arabicText}
      </div>
      <div className={`tafsir-mcq-choices ${showFeedback ? 'feedback-shown' : ''}`}>
        {shuffledOptions.map((option) => {
          const isTapped = currentAnswer === option;
          const isCorrectAnswer = option === correctAnswer;
          let className = 'tafsir-choice-btn';
